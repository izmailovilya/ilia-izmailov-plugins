...
| Proxy teammate: `ENGINE RUNNING: {role} on {engine}, started {HH:MM}, expect ~{N} min` | Note the role, engine and expected finish time in state.md. No other action — this is the clock you judge silence by. | `⚙️ {role} работает на {engine}, это займёт примерно {N} мин.` |
...
| A proxy teammate is silent past its `ENGINE RUNNING` estimate | Same rule — ask, don't replace. Check `.claude/teams/{team}/engine/{role}/` first: the prompt, session id and output file are written as the run proceeds, so they tell you how far it got without disturbing anyone. If the output file is complete but the proxy never reported, ask it to report; if the session id is missing, the engine conversation cannot be resumed and the round must be restarted from the prompt file. |
